...
class Config(QConfig):
    ...
    # folders
    transcriptionFolders = ConfigItem(
        "Folders", "LocalTranscription", [], FolderListValidator())
    downloadFolder = ConfigItem(
        "Folders", "Download", "download", FolderValidator())

    # local
    transcriptionModel = OptionsConfigItem(
        "Local", "TranscriptionModel", "Local SenseVoice",
            OptionsValidator(["Local SenseVoice", "SenseVoice API", "Google"]))
    summaryLLMModel = OptionsConfigItem(
        "Local", "SummaryLLMModel", "DeepSeek-V3",
            OptionsValidator(["DeepSeek-V3", "DeepSeek-R1", "GPT-4o"]))
